...
void CWENOFD::initializeSolver(std::map<std::string, std::string> option)
{
    ...
    int basePointNumX = m_worldPointNumX / m_size; // 每个线程基本分配元素数
    int remainder = m_worldPointNumX % m_size;     // 剩余元素数

    if (m_rank == m_size - 1)
        // 最后一个线程多分配剩余元素
        m_numPointsX = basePointNumX + remainder;
    else
        m_numPointsX = basePointNumX;
...
void CWENOFD::getFlux(void)
{
    ...
    for (int ej = m_startPointY; ej != m_endPointY; ej++)
    {
        for (int ei = 0; ei != m_totalPointNumX; ei++)
        {
            fluxSplit(m_Uh[ei][ej].vector, 1, 0, loc_flux_minus, loc_flux_plus);
            for (int r = 0; r != m_varNum; ++r)
            {
    ...
    for (int ei = m_startPointX; ei != m_endPointX; ei++)
    {
        ...
        for (int ej = 0; ej != m_totalPointNumY; ej++)
        {
            ...
            fluxSplit(m_Uh[ei][ej].vector, 0, 1, loc_flux_minus, loc_flux_plus); // y 方向 unit normal: (0,1)
...
void CWENOFD::outputAve(std::string prefix)
{
    ...
    if (m_rank == 0)
    {
        ...
        // Output file name
        std::string filename = "./output/average_" + prefix + ".plt";
...
void CWENOFD::outputError(std::string prefix)
{
    ...
    if (m_rank == 0)
    {
        ...
        // Output file name
        std::string filename = "./output/error_" + prefix + ".plt";
        std::ofstream outputFile(filename);
...
void CWENOFD::outputAccuracy(std::string prefix)
{
    ...
    if (m_rank == 0)
    {
        // Output to file
        std::fstream fileout;
        std::string filename = "./output/accuracy_" + prefix + ".csv";
